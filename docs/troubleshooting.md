...
`/reportstatus` 和 `/reportretry` 要求管理员权限；从 v0.3.1 起 `/reportnow` 对普通玩家开放。新代码遇到 `40034105` 后暂停对应 Bot 的主动播报，之后不会反复生成报告和调用被拒绝的发送接口。暂停状态保存在当前进程内，重启后会重新尝试；希望一直关闭时请使用上述环境配置。
